...
- **Docked layout structure.** The current UI is a docked layout (TopBar, LeftToolbar, RightPanel, StatusBar around a full-bleed canvas). Legacy Pebble UI (floating panels, warm earth tones) is fully deprecated — do not introduce new floating-panel layouts. Use the component patterns in `docs/design-system.md` §7 (Button, Floating Panel (popover/overlay component, not a layout pattern), Input Field, Color Swatch) for individual elements.
