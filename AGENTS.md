...
| Testing | Swift Testing | `apple/DotorixelTests/`; docked view snapshots via swift-snapshot-testing on a pinned iOS sim — see `apple/DotorixelTests/README.md` |
...
3. Classify the item and route to the appropriate skill (to-spec / to-tickets / tdd).
...
- **Exception — non-code tasks**: Tasks that don't modify production code or tests (e.g., `.pen` design files, `issues/*.md`, `tasks/*.md`, `docs/`, `CONTEXT.md`, `AGENTS.md`, `.agents/skills/`, `.claude/skills/`) do not require a work branch or PR. Commit directly to `main`.
