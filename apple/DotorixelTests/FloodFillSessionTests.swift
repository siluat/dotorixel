...
struct FloodFillSessionTests {
    ...
    func tapFillsEnclosedRegion() throws {
        ...
        for y in 0..<8 {
            try state.document.setPixel(x: 3, y: UInt32(y), color: red)
        }
        ...
        #expect(try state.document.getPixel(x: 2, y: 7) == fg)
        // …the wall keeps its color and the right region stays untouched.
        #expect(try state.document.getPixel(x: 3, y: 4) == red)
        #expect(try state.document.getPixel(x: 4, y: 4) == transparent)
        #expect(try state.document.getPixel(x: 7, y: 7) == transparent)
    }
    ...
    func dragAfterTapFillsNothingFurther() throws {
        ...
        #expect(try state.document.getPixel(x: 5, y: 4) == transparent)
    ...
    func sameColorFillIsNoOp() throws {
        ...
        let fg = state.foregroundColor
        try state.document.setPixel(x: 2, y: 2, color: fg)
        try state.document.setPixel(x: 2, y: 3, color: fg)
        state.activeTool = .floodFill
        let pixelsBefore = state.document.composite()
        let versionBefore = state.canvasVersion

        state.beginStroke(at: ScreenCanvasCoords(x: 2, y: 2))
        state.endStroke()

        #expect(state.document.composite() == pixelsBefore)
        #expect(state.canvasVersion == versionBefore)
    ...
    func noopFillPreservesRedoFuture() throws {
        ...
        #expect(state.canRedo)
        state.handleRedo()
        #expect(try state.document.getPixel(x: 0, y: 0) == state.foregroundColor)
    }
    ...
    func oneUndoRevertsWholeFill() throws {
        ...
        #expect(try state.document.getPixel(x: 5, y: 5) == red)
        #expect(paintedPixelCount(state) == 1)

        state.handleRedo()
        #expect(try state.document.getPixel(x: 0, y: 0) == fg)
        #expect(try state.document.getPixel(x: 7, y: 7) == fg)
        #expect(try state.document.getPixel(x: 5, y: 5) == red)
    }
    ...
    func secondaryButtonFillsBackground() throws {
        ...
        #expect(try state.document.getPixel(x: 0, y: 0) == state.backgroundColor)
        #expect(try state.document.getPixel(x: 7, y: 7) == state.backgroundColor)
    }
