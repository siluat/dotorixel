...
mod tests {
    ...
    fn with_position_returns_placement_with_new_position_and_unchanged_scale() {
        let placement = ReferencePlacement { x: 10.0, y: 20.0, scale: 2.0 };
        let moved = placement.with_position(-5.5, 7.25);
        assert_eq!(moved, ReferencePlacement { x: -5.5, y: 7.25, scale: 2.0 });
    }
    ...
    fn with_scale_returns_placement_with_new_scale_and_unchanged_position() {
        ...
        assert_eq!(scaled, ReferencePlacement { x: 10.0, y: 20.0, scale: 3.5 });
    }

    #[test]
    fn restore_to_natural_resets_scale_to_one_and_preserves_center_when_scaled_up() {
        let original = ReferencePlacement { x: 10.0, y: 20.0, scale: 2.0 };
    ...
    fn restore_to_natural_preserves_center_when_scaled_down() {
        let original = ReferencePlacement { x: 0.0, y: 0.0, scale: 0.5 };
    ...
    fn projected_center(p: ReferencePlacement, natural_width: u32, natural_height: u32) -> (f32, f32) {
        (
    ...
    fn builders_leave_original_placement_unchanged() {
        ...
        assert_eq!(original, ReferencePlacement { x: 10.0, y: 20.0, scale: 2.0 });
    }
}
