...
pub fn flood_fill(
    ...
    let ux = start_x as u32;
    let uy = start_y as u32;
    if !canvas.is_inside_bounds(ux, uy) {
        return false;
    }
    ...
    visited[uy as usize * w + ux as usize] = true;
    ...
    while let Some((cx, cy)) = queue.pop_front() {
        ...
        for (dx, dy) in DIRS {
            ...
            let nux = nx as u32;
            let nuy = ny as u32;
            if !canvas.is_inside_bounds(nux, nuy) {
                continue;
            }
...
mod tests {
    ...
    fn flood_fill_single_pixel() {
        ...
        // Paint one pixel red, then flood-fill it to black
        canvas.set_pixel(1, 1, RED).unwrap();
        assert!(flood_fill(&mut canvas, 1, 1, BLACK));
        assert_eq!(canvas.get_pixel(1, 1).unwrap(), BLACK);
        // Adjacent transparent pixels are not affected
    ...
    fn flood_fill_entire_canvas() {
        let mut canvas = PixelCanvas::new(4, 4).unwrap();
        // All pixels are transparent — fill entire canvas with RED
        assert!(flood_fill(&mut canvas, 0, 0, RED));
        for y in 0..4 {
            for x in 0..4 {
    ...
    fn flood_fill_respects_4_connectivity() {
        // Diagonal pixels of a different color should NOT be filled
        let mut canvas = PixelCanvas::with_color(4, 4, BLACK).unwrap();
        // Create a small region of RED that only connects diagonally to (0,0)
    ...
    fn flood_fill_stops_at_boundary() {
        // Create a barrier that prevents fill from spreading
        let mut canvas = PixelCanvas::new(4, 4).unwrap();
        // Build a wall of RED pixels across column 2
        for y in 0..4 {
            canvas.set_pixel(2, y, RED).unwrap();
        }
        // Fill left side with BLACK starting at (0,0)
        assert!(flood_fill(&mut canvas, 0, 0, BLACK));
        ...
        // Right side remains transparent
    ...
    #[test]
    fn flood_fill_negative_x_returns_false() {
        let mut canvas = PixelCanvas::new(4, 4).unwrap();
        assert!(!flood_fill(&mut canvas, -1, 0, RED));
    }

    #[test]
    fn flood_fill_negative_y_returns_false() {
        let mut canvas = PixelCanvas::new(4, 4).unwrap();
        assert!(!flood_fill(&mut canvas, 0, -1, RED));
    }

    #[test]
    fn flood_fill_out_of_bounds_returns_false() {
        let mut canvas = PixelCanvas::new(4, 4).unwrap();
        assert!(!flood_fill(&mut canvas, 4, 0, RED));
        assert!(!flood_fill(&mut canvas, 0, 4, RED));
    }
}
