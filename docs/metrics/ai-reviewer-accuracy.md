...
| #367 | coderabbitai[bot] | Miss | Re-reviewed the follow-up with no actionable findings and missed the translucent-Pixel source-over mismatch |
...
| #355 | greptile-apps[bot] | Reject | Flagged the Marquee missing from `TabSnapshot`/persistence as a P1; intentional scope split — Marquee persistence is issue 276, blocked by 272 because it must also flush Floating Selection state |
| #355 | greptile-apps[bot] | Miss | Missed the Reduce Motion live-toggle gap (accepted from coderabbit/cubic) |
...
| #348 | coderabbitai[bot] | Reject | Wanted "prefactor" corrected to "refactor" in the metrics log; "prefactor" is the repo's established vocabulary — issue 262 is titled "Workspace state split (prefactor)" and its body, todo.md, and the parent RFC all use it |
| #348 | greptile-apps[bot] | Miss (×2) | Re-reviewed bdd1760 at 5/5 with no findings; missed the stranded-tab begin overwrite and the 264 contradiction |
| #348 | cubic-dev-ai[bot] | Miss | Flagged only the 264 contradiction; missed the stranded-tab begin overwrite (accepted from coderabbit) |
...
| #342 | cubic-dev-ai[bot] | Reject | `maxSnapshots == 0` panics across FFI — the core documents 0 as a caller error by contract, the binding mirrors the existing AppleHistoryManager::new shape, and the only caller passes compile-time constants |
| #342 | greptile-apps[bot] | Miss | Did not flag the uncovered binding-only layer-id error paths (accepted from coderabbit) |
| #342 | cubic-dev-ai[bot] | Miss | Did not flag the uncovered binding-only layer-id error paths (accepted from coderabbit) |
...
| #341 | cubic-dev-ai[bot] | Accept | Same "Validated by the 255 device pass" completed-vs-pending phrasing (duplicate of coderabbit); already resolved by c344eff before cubic's comment was triaged |
...
| #341 | cubic-dev-ai[bot] | Miss | Did not flag the missing CONTEXT.md Hover Gate documentation (accepted from greptile) |
