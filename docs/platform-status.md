...
| Reference Layer (timeline kind) | 🔧 | ⬜ | ⬜ | Layer kind for tracing imagery. Core has `add_reference_layer` (aspect-preserving auto-fit placement, `scale ≤ 1.0`), kind-aware accessors, and drawing-method defenses (`set_pixel`/`get_pixel` → `DrawError::LayerKindMismatch`; `apply_tool`/`flood_fill`/`clear` → no-op). Excluded from exports. Shell wiring lands via 113 (WASM facade) and beyond |
