...
| Color sampling — Eyedropper | — | ✅ | ⬜ | When the Eyedropper tool is active, tapping a reference window's image samples the pixel under the pointer into the foreground color. Pure `samplePixel(DecodedImage, x, y)` extractor + pure `windowToImageCoords` (floor-then-clamp defends trailing-edge off-by-one) + thin async `sampler(blob, x, y)` boundary using `createImageBitmap` → `OffscreenCanvas.getImageData` (stateless, no cache); `TabState.sampleReferencePixel` routes through the existing `#applyEffects` dispatcher to emit `colorPick(foreground) + addRecentColor + markDirty` for opaque samples; transparent (a===0) and decode failure are silent. Letterbox region around the image naturally falls through to z-order activation since `onpointerdown` is bound to the `<img>` element. Long-press sampling for touch and drag-to-refine remain future work |
