...
| Eyedropper | — | ✅ | ✅ | Drag-and-commit to FG/BG; skips transparent and out-of-bounds samples. Apple samples the Pixel composite source-over the visible Reference; Web samples the active layer. |
...
| Selection / Marquee | ✅ | ✅ | ✅ | Web and Apple: Marquee/Floating, clipping, transforms, clipboard, keyboard/touch actions, persistence, and Reference-active hide/no-op semantics. |
...
| Frame cel-grid | ✅ | ✅ | 🔧 | One Cel per Pixel Layer per frame (grid invariant); Reference frame-independent. Undoable add/duplicate/remove on both shells; switching the Active Frame is never a History entry and commits a pending Floating Selection first. Web: drag reorder + multi-frame V7 persistence. Apple: reorder and multi-frame persistence pending |
...
| Timeline panel | — | 🔧 | 🔧 | Bottom-docked Layer × Frame grid on both, with a horizontally scrolling frame axis under a pinned ruler. Apple: layer sidebar with drag reorder, fixed Reference band, Cel occupancy dots, header frame actions (a menu where the row won't fit), transport slot reserved. Web mobile row targets pending |
...
| Display on canvas | — | ✅ | ✅ | Web uses persistent movable multi-window references. Apple renders one underlay below pixels, positioned on-canvas by a placement overlay; placement and visibility survive relaunch |
