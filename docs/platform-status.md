...
| Timeline panel — desktop shell | — | 🔧 | ⬜ | `TimelinePanel.svelte` in the docked layout's `timeline` row (180px). Sidebar (256px) lists layers top-z first (Aseprite/Photoshop convention) with accent bar + bold on the active row; row click (or Space/Enter) activates. Header `+` adds a layer; per-row `✕` removes it (disabled at single-layer). Placeholder frame column reserves the M4 ruler region. Mobile entry pending (#098). Not yet on Apple |
