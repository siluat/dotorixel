...
| Stroke Baseline (no-op discard) | ✅ | ✅ | ✅ | Stroke undo entries commit at stroke end only if pixels changed; no-op strokes (out-of-canvas, same-color, cancelled, zero-delta move) leave History untouched and preserve redo. Core-owned comparison |
