...
| Edit Baseline (no-op discard) | ✅ | ✅ | ✅ | Commits an undo entry at an Edit's end only if the state changed (Apple: pixels, Web: whole Document). No-op Edits — strokes and commands alike — leave History untouched and preserve redo. Core-owned comparison |
