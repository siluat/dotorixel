...
| Pixel-perfect filter | ✅ | 🔧 | ⬜ | L-corner 3-window rule (Aseprite-style); stateless with `TailState` carry; WASM + UniFFI bindings exported. Web: pencil/eraser strokes wired through `createPixelPerfectOps` decorator (hardcoded ON; toggle UI pending) |
