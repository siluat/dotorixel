...
| Onion skinning | — | ✅ | 🔧 | Adjacent-frame ghosts while drawing (prev/next 1, clamped, no wrap): prev warm / next cool, hidden during Playback, never in exports. Apple: state + ghost buffers only — render/toggle UI and toggle persistence pending |
