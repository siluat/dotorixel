...
| `src/routes/editor/+page.svelte` | `displayReference` now calls `store.nextCascadeIndex(docId)` instead of inlining `displayStatesForDoc(docId).length` (the inlined version counted hidden states forever and never reset). Bug fix in `handleReferenceSelect`: clicking a *displayed* card no longer early-returns — it always raises (`store.show`) before closing the modal, matching the spec's "explicit show action". |
...
- **`store.show()` is the single path for both re-display and raise-to-top.** It already sets `visible: true` and bumps `zOrder` above the current max, so it works idempotently for an already-visible window (just bumps z-order).
