...
| `src/lib/canvas/sampling-session.svelte.test.ts` | Added 5 new behavior tests: preserve centerColor on null update, preserve on transparent update, refuse commit when drifted onto transparent, refuse commit when drifted out of canvas, and commit the final opaque color when a drag crosses transparent back to opaque. |
