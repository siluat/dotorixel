...
1. **Timeline Panel additions** — a Reference Layer add icon next to the existing `+` (Pixel add); a per-row kind icon distinguishing Pixel vs Reference; the Fit to canvas inline action affordance (on row or near the overlay — pick one in the design and document); a loading skeleton row state.
...
- Fit to canvas action affordance finalized — placement (on row or overlay) decided and documented in the `.pen`.
...
- **Fit to canvas location** — inline on the row, **only when the row is `active && kind = Reference`**. Sits to the left of the delete `×`.
- **Fit to canvas glyph** — Lucide `maximize-2` (diagonal corner arrows). Conveys "fit/expand to frame" without colliding with the generic undo metaphor.
...
- §2 Per-row kind icon — light + dark, 4 row variants per theme (Pixel inactive/active, Reference inactive/active with Fit icon).
...
- Fit to canvas: inline on row, **only when `active && kind = Reference`**, left of delete `×`, Lucide `maximize-2`.
...
PRD-105 was corrected after implementation review: Reference Layer is now a singleton fixed-bottom underlay, not a reorderable layer in the composited pixel stack. The visual design remains useful for the import icon, kind icon, fit action, loading state, and placement overlay, but implementation must adapt the Timeline row anatomy:
...
Implementation planning replaced the low-value "Restore original size" row action with "Fit to canvas". The row location and Lucide `maximize-2` glyph stay the same; the action now means aspect-fit the source into the current canvas, centered, with upscaling allowed.
