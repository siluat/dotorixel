...
export interface KeyboardInputHost {
	...
	cutSelection(): void;
	/** Paste the workspace Selection Clipboard into the active document. */
...
export interface KeyboardInput {
	...
	/**
	 * Called by the Input Pipeline when a draw stroke ends or is canceled to restore a temporary tool switch.
...
export function createKeyboardInput(host: KeyboardInputHost): KeyboardInput {
	...
	return {
		handleKeyDown(event: KeyboardEvent): void {
			...
			if (isCtrlOrCmd && !event.altKey && isVKey) {
				event.preventDefault();
				if (event.repeat) return;
				if (host.isDrawing()) return;
