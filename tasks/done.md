...
| `src/routes/+page.server.ts` | Server-side 307 redirect `/` → `/pebble` |
...
| `src/lib/ui-pixel/*.stories.svelte` | `.pixel-editor .pixel-story-bg` wrapper added to all 12 story files |

#### Key Decisions
- Server-side redirect (`+page.server.ts`) instead of client-side (`+page.ts`) to avoid racing with layout's `initWasm()`
